...
def get_result(rid: str) -> Optional[Dict]:
    ...
    conn = _get_conn()
    row = conn.execute(
        "SELECT full_json FROM analysis_results WHERE id = ?", (rid,)
    ).fetchone()
...
def get_latest(source: str = None) -> Optional[Dict]:
    ...
    if source:
        ...
        row = conn.execute(
            "SELECT full_json FROM analysis_results ORDER BY created_at DESC LIMIT 1"
        ).fetchone()
    return json.loads(row["full_json"]) if row else None
...
def get_model_scan(rid: str) -> Optional[Dict]:
    ...
    conn = _get_conn()
    row = conn.execute(
        "SELECT full_json FROM model_scans WHERE id = ?", (rid,)
    ).fetchone()
    return json.loads(row["full_json"]) if row else None
...
def get_stats() -> Dict:
    ...
    by_source = conn.execute(
        "SELECT source, COUNT(*) as n FROM analysis_results GROUP BY source"
    ).fetchall()
    by_verdict = conn.execute(
        "SELECT verdict, COUNT(*) as n FROM analysis_results GROUP BY verdict"
    ).fetchall()
